...
class _WeeklyGoalsScreenState extends State<WeeklyGoalsScreen> {
  ...
  Future<void> _addGoal() async {
    ...
    try {
      // Use the currently selected week start
      final goalDate = DateTime(_currentWeekStart.year, _currentWeekStart.month,
          _currentWeekStart.day, 12, 0, 0);
  ...
  Widget build(BuildContext context) {
    ...
    // Calculate week number using _currentWeekStart
    final startOfYear = DateTime(_currentWeekStart.year, 1, 1);
    final weekNumber = ((_currentWeekStart.difference(startOfYear).inDays +
                startOfYear.weekday) /
            7)
        .ceil();
    final endOfWeek = _currentWeekStart.add(const Duration(days: 6));
    final weekRange =
        '${_currentWeekStart.month}/${_currentWeekStart.day} - ${endOfWeek.month}/${endOfWeek.day}';
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Set a direction for your week',
                            style: TextStyle(
